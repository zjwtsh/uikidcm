...
int SsbSipJPEGDecodeInit(void)
{
	...
	if (jCtx->mappedAddr == NULL) {
		LOG_MSG(LOG_ERROR, "SsbSipJPEGDecodeInit", "JPEG mmap failed %08X \n",jCtx->mappedAddr);
		return -1;
	}	LOG_MSG(LOG_ERROR, "SsbSipJPEGDecodeInit", "JPEG mmap failed %08X \n",jCtx->mappedAddr);
...
int SsbSipJPEGEncodeInit(void)
{
	...
	if (jCtx->mappedAddr == NULL) {
		LOG_MSG(LOG_ERROR, "SsbSipJPEGEncodeInit", "JPEG mmap failed %08X\n",jCtx->mappedAddr);
		return -1;
	}
...
//	LOG_MSG(LOG_TRACE, "SsbSipJPEGDecodeExe", "jCtx->decParam->fileSize : %d\n", jCtx->decParam->fileSize);
	...
	if(jCtx->decParam->decType == JPG_MAIN){
		ccc = ioctl(dev_fd, IOCTL_JPG_DECODE, jCtx->decParam);//		LOG_MSG(LOG_TRACE, "SsbSipJPEGDecodeExe", "ioctl : %d\n", ccc);
//
...
-----------------------------------------------------------------------------*/
char *SsbSipJPEGGetDecodeInBuf(int dev_fd, long size)
{
...
//	printf( "SsbSipJPEGGetDecodeInBuf Size %d\n",size);
...
-----------------------------------------------------------------------------*/
char *SsbSipJPEGGetDecodeOutBuf (int dev_fd, long *size)
{
