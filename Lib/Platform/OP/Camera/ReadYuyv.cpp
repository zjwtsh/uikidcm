...
static int ReadFileCleanup(
    int ret, unsigned char **buffer, int *fileSize, int fd)
{
  if (ret) {
    if (*buffer) {
      free(*buffer);
...
static int ReadImgFile(char *fileName, unsigned char **buffer, int *fileSize)
{
  int ret = 0;
  struct stat statbuf;
  int fd = -1;
