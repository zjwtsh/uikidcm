...
int lua_ball_color_stats(lua_State *L) {
  ...
  if (lua_gettop(L) >= 5) {
	...
	// initialize statistics
	int area = 0;
  int whiteArea = 0, othersArea = 0, blackArea = 0;
	int minI = width-1, maxI = 0;
	int minJ = height-1, maxJ = 0;
	int sumI = 0, sumJ = 0;
	int sumII = 0, sumJJ = 0, sumIJ = 0;
  ...
  lua_pushstring(L, "blackArea");
  lua_pushnumber(L, blackArea);
  lua_settable(L, -3);

  lua_pushstring(L, "whiteArea");
  lua_pushnumber(L, whiteArea);
  lua_settable(L, -3);

  lua_pushstring(L, "othersArea");
  lua_pushnumber(L, othersArea);
  lua_settable(L, -3);
